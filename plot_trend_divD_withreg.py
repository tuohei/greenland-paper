...
#d = xr.open_dataset(f'{dirpath}greenland_divD.WN1-3.summed.nc')
dirpath = '/mnt/data/greenland/decor/summed/'
#d3 = xr.open_dataset(f'{dirpath}greenland_divD.WN4-5.summed.nc')
#d = xr.open_dataset(f'{dirpath}divD.WN0-3.1979-2018.smoothed.addedWN4-5.anom.south_west.to_smb.summed.nc')
#d3 = xr.open_dataset(f'{dirpath}divD.WN4-5.1979-2018.smoothed.decorWN0-3.anom.south_west.to_smb.summed.nc')
#d4 = xr.open_dataset(f'{dirpath}divD.WN0-5.1979-2018.smoothed.anom.south_west.to_smb.summed.nc')
#d2 = xr.open_dataset(f'{dirpath}south_west_SMB_Icemask.summed.nc')
d = xr.open_dataset(f'{dirpath}divD.WN0-3.1979-2018.smoothed.addedWN4-5.anom.to_smb.summed.nc')
d3 = xr.open_dataset(f'{dirpath}divD.WN4-5.1979-2018.smoothed.decorWN0-3.anom.to_smb.summed.nc')
d4 = xr.open_dataset(f'{dirpath}divD.WN0-5.1979-2018.smoothed.anom.to_smb.summed.nc')
...
#d.divD.data /= np.max(np.abs(d.divD.data))
...
divDmax = max([np.nanmax(np.abs(d_rolled.data[:,0,0])),
...
df['divDP'] = d_rolled.data[ndays_rolling:,0,0]/divDmax
#/divDmax #np.nanmax(np.abs(d_rolled.data[:,0,0]))

df['divDS'] = d3_rolled.data[ndays_rolling:,0,0]/divDmax
df['divD'] = d4_rolled.data[ndays_rolling:,0,0]/divDmax
#/divDmax #np.nanmax(np.abs(d3_rolled.data[:,0,0]))
df['smb'] = df2['smb']
print(df.corr())
#df['divD'] = df['divDS'] + df['divDP']
#slope,intercept,rvalue,pvalue,stderr = linregress(df['divDP'],df['smb'])
#print(slope,intercept)
#df['divDP'] *= slope
#df['divDP'] += intercept

#slope,intercept,rvalue,pvalue,stderr = linregress(df['divDS'],df['smb'])
#df['divDS'] *= slope
#df['divDS'] += intercept
#slope,intercept,rvalue,pvalue,stderr = linregress(df['divD'],df['smb'])
#df['divD'] *= slope
#df['divD'] += intercept
...
p1, = ax.plot(df.index,df['divDP'],label = 'divDP',color = 'red')#,linestyle = '--')
p2, = ax.plot(df.index,df['divDS'],label = 'divDS',color = 'blue')#,linestyle = 'dotted')
p3, = ax.plot(df.index,df['divD'],label = 'divD',color = 'grey')#, linestyle = '-.')

#sns.lineplot(data = df2,linewidth = 2.5,ax = ax2)
ax.set_yticks(np.linspace(-1,1,11))
ax.set_yticklabels(np.round(divscaling*np.linspace(-divDmax,divDmax,11),0))
...
divDtrends = df3.mean() - df1.mean()
print(smb_trend)
print(divDtrends)
divDtrends/= smb_trend['smb']
divDtrends*=100
print(divDtrends)
...
w = 0.2
centre = 0.5
bar1 = axins.bar(centre - w, divDtrends['divDP'],width = w,color = 'red')
bar2 = axins.bar(centre , divDtrends['divDS'],width = w,color = 'blue')
bar3 = axins.bar(centre + w, divDtrends['divD'],width = w,color = 'grey')
...
fig1.savefig(f'lineplots_{years}yrmean_1979-2017.smooth10days.greenland.D.png')
fig1.savefig(f'lineplots_{years}yrmean_1979-2017.smooth10days.greenland.D.eps')
#fig1.savefig('reg_lineplots_10yrmean_1979-2017.smooth10days.south_west.Q.png')
...
#sns.regplot(x ='divDP',y = 'smb',data = df,ax = ax1,color = 'b',scatter_kws = {'alpha': .1,'color': 'k'})
#ax2 = fig.add_subplot(gs[0,1])
#sns.regplot(x = 'divDS',y = 'smb',data = df,ax = ax2,color = 'g',scatter_kws = {'alpha': .1,'color': 'k'})
