...
d = xr.open_dataset(f'{dirpath}south_west_divD_divQ_WN0-3_WN4-5.to_smb.summed.nc')
...
p1, = ax.plot(df.index,df['divQP'],label = '(a)',color = 'red')#,linestyle = '--')
p2, = ax.plot(df.index,df['divQS'],label = '(b)',color = 'blue')#,linestyle = 'dotted')
p3, = ax.plot(df.index,df['divQ'],label = '(c)',color = 'grey')#, linestyle = '-.')

#sns.lineplot(data = df2,linewidth = 2.5,ax = ax2)
numticks = 15
ax.set_yticks(np.linspace(-1,1,numticks))
ax.set_yticklabels(np.round(divscaling*np.linspace(-divQmax,divQmax,numticks),0))
ax.set_ylim(-1,1)
ax.set_ylabel(f'{1/divscaling:.0E} x mm w.e. (div)')
p4, = ax2.plot(df2.index,df2['smb'],label = 'smb',color = 'k')
ax2.set_yticks(np.linspace(-1,1,numticks))
ax2.set_yticklabels(np.round(smbscaling*np.linspace(-smbmax,smbmax,numticks),0))
...
fig1.savefig(f'figures/lineplots_{years}yrmean_1979-2017.smooth10days.greenland.totenergy.png')
fig1.savefig(f'figures.lineplots_{years}yrmean_1979-2017.smooth10days.greenland.totenergy.eps')
